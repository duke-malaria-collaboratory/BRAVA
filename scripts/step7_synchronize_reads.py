...
def makeOutDirs(out):
    ...
    print("\tSynchronized paired end fastq files stored in {}".format(out))
    os.system('mkdir -p {}/map_info'.format(out))
    print("\tLog files stored in {}/map_info".format(out))
...
def splitReads(refSeqs, refNames, out, read1, read2, read1Dir, read2Dir, target):
    ...
    for elem in refNames:
        ...
        os.system("gzip {0}/*.fastq".format(out))
        if not os.path.exists(out + "/1"):
            os.system("mkdir {}/1".format(out))
        if not os.path.exists(out + "/2"):
            os.system("mkdir {}/2".format(out))
        os.system("mv {0}/*_1.fastq.gz {0}/1".format(out))
        os.system("mv {0}/*_2.fastq.gz {0}/2".format(out))
    print("Synchronizing paired end reads...\n")
    for elem in refNames:
        unsyncReads1 = getReads("{}/1".format(out))
        unsyncReads2 = getReads("{}/2".format(out))
        syncReads("{}/1".format(out), "{}/2".format(out), unsyncReads1, unsyncReads2)
