...
snakemake <- Snakemake(
    ...
    output = list('out/haplotype_output', 'out/haplotype_output/AMA_trimAndFilterTable', 'out/haplotype_output/AMA_haplotypes.rds', 'out/haplotype_output/AMA_trackReadsThroughPipeline.csv', "out" = c('out/haplotype_output'), "trim_filter_table" = c('out/haplotype_output/AMA_trimAndFilterTable'), "results" = c('out/haplotype_output/AMA_haplotypes.rds'), "reads_table" = c('out/haplotype_output/AMA_trackReadsThroughPipeline.csv')),
    params = list(c('out/fastq/AMA/all_samples'), "all_samples" = c('out/fastq/AMA/all_samples')),
    wildcards = list(),
    threads = 1,
    log = list(),
    resources = list(),
    config = list("ncores" = 4, "pair1" = 'raw_reads/1', "target" = 'AMA', "reverse" = 'adapters/reversePrimers', "out" = 'out', "pair2" = 'raw_reads/2', "forward" = 'adapters/forwardPrimers', "refs" = 'refs')
)
######## Original script #########
# ----------------------------------------- #
