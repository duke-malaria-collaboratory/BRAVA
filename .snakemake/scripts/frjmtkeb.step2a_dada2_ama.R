...
snakemake <- Snakemake(
    ...
    output = list('out/haplotype_output', 'out/haplotype_output/AMA_trimAndFilterTable', 'out/haplotype_output/AMA_trackReadsThroughPipeline.csv', 'out/haplotype_output/AMA_haplotypes.rds', "out" = c('out/haplotype_output'), "trim_filter_table" = c('out/haplotype_output/AMA_trimAndFilterTable'), "reads_table" = c('out/haplotype_output/AMA_trackReadsThroughPipeline.csv'), "results" = c('out/haplotype_output/AMA_haplotypes.rds')),
    params = list(c('out/fastq/AMA/all_samples'), "all_samples" = c('out/fastq/AMA/all_samples')),
    wildcards = list(),
    threads = 1,
    log = list(),
    resources = list(),
    config = list("out" = 'out', "ncores" = 4, "target" = 'AMA', "pair2" = 'raw_reads/2', "refs" = 'refs', "pair1" = 'raw_reads/1', "forward" = 'adapters/forwardPrimers', "reverse" = 'adapters/reversePrimers')
)
######## Original script #########
# ----------------------------------------- #
