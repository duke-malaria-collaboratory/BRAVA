...
snakemake <- Snakemake(
    ...
    output = list('out/haplotype_output/AMA_trackReadsThroughPipeline.csv', 'out/haplotype_output', 'out/haplotype_output/AMA_trimAndFilterTable', 'out/haplotype_output/AMA_haplotypes.rds', "trim_filter_table" = c('out/haplotype_output/AMA_trimAndFilterTable'), "out" = c('out/haplotype_output'), "reads_table" = c('out/haplotype_output/AMA_trackReadsThroughPipeline.csv'), "results" = c('out/haplotype_output/AMA_haplotypes.rds')),
    params = list(c('out/fastq/AMA/all_samples'), "all_samples" = c('out/fastq/AMA/all_samples')),
    wildcards = list(),
    threads = 1,
    log = list(),
    resources = list(),
    config = list("target" = 'AMA', "forward" = 'adapters/forwardPrimers', "refs" = 'refs', "pair2" = 'raw_reads/2', "pair1" = 'raw_reads/1', "ncores" = 4, "out" = 'out', "reverse" = 'adapters/reversePrimers')
)
######## Original script #########
# ----------------------------------------- #
