...
snakemake <- Snakemake(
    input = list('scripts/step2a_dada2_ama.R', "R" = c('scripts/step2a_dada2_ama.R')),
    output = list('out/haplotype_output/AMA_trimAndFilterTable', 'out/haplotype_output/AMA_haplotypes.rds', 'out/haplotype_output/AMA_trackReadsThroughPipeline.csv', 'out/haplotype_output', "out" = c('out/haplotype_output'), "results" = c('out/haplotype_output/AMA_haplotypes.rds'), "reads_table" = c('out/haplotype_output/AMA_trackReadsThroughPipeline.csv'), "trim_filter_table" = c('out/haplotype_output/AMA_trimAndFilterTable')),
    params = list(c('out/fastq/AMA/all_samples'), "all_samples" = c('out/fastq/AMA/all_samples')),
    wildcards = list(),
    threads = 1,
    log = list(),
    resources = list(),
    config = list("forward" = 'adapters/forwardPrimers', "pair1" = 'raw_reads/1', "ncores" = 4, "refs" = 'refs', "pair2" = 'raw_reads/2', "target" = 'AMA', "out" = 'out', "reverse" = 'adapters/reversePrimers')
)
######## Original script #########
# ----------------------------------------- #
