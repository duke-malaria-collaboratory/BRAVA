...
snakemake <- Snakemake(
    ...
    output = list('out/haplotype_output/AMA_haplotypes.rds', 'out/haplotype_output/AMA_trimAndFilterTable', 'out/haplotype_output/AMA_trackReadsThroughPipeline.csv', 'out/haplotype_output', "results" = c('out/haplotype_output/AMA_haplotypes.rds'), "out" = c('out/haplotype_output'), "reads_table" = c('out/haplotype_output/AMA_trackReadsThroughPipeline.csv'), "trim_filter_table" = c('out/haplotype_output/AMA_trimAndFilterTable')),
    params = list(c('out/fastq/AMA/all_samples'), "all_samples" = c('out/fastq/AMA/all_samples')),
    wildcards = list(),
    threads = 1,
    log = list(),
    resources = list(),
    config = list("refs" = 'refs', "ncores" = 4, "forward" = 'adapters/forwardPrimers', "out" = 'out', "target" = 'AMA', "pair2" = 'raw_reads/2', "reverse" = 'adapters/reversePrimers', "pair1" = 'raw_reads/1')
)
######## Original script #########
# ----------------------------------------- #
