...
snakemake <- Snakemake(
    ...
    output = list('out/haplotype_output', 'out/haplotype_output/AMA_trimAndFilterTable', 'out/haplotype_output/AMA_trackReadsThroughPipeline.csv', 'out/haplotype_output/AMA_haplotypes.rds', "trim_filter_table" = c('out/haplotype_output/AMA_trimAndFilterTable'), "reads_table" = c('out/haplotype_output/AMA_trackReadsThroughPipeline.csv'), "out" = c('out/haplotype_output'), "results" = c('out/haplotype_output/AMA_haplotypes.rds')),
    params = list(c('out/fastq/AMA/all_samples'), "all_samples" = c('out/fastq/AMA/all_samples')),
    wildcards = list(),
    threads = 1,
    log = list(),
    resources = list(),
    config = list("ncores" = 4, "refs" = 'refs', "pair1" = 'raw_reads/1', "pair2" = 'raw_reads/2', "reverse" = 'adapters/reversePrimers', "out" = 'out', "target" = 'AMA', "forward" = 'adapters/forwardPrimers')
)
######## Original script #########
# ----------------------------------------- #
