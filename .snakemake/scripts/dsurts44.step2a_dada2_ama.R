...
snakemake <- Snakemake(
    ...
    output = list('out/haplotype_output', 'out/haplotype_output/AMA_haplotypes.rds', 'out/haplotype_output/AMA_trimAndFilterTable', 'out/haplotype_output/AMA_trackReadsThroughPipeline.csv', "out" = c('out/haplotype_output'), "results" = c('out/haplotype_output/AMA_haplotypes.rds'), "reads_table" = c('out/haplotype_output/AMA_trackReadsThroughPipeline.csv'), "trim_filter_table" = c('out/haplotype_output/AMA_trimAndFilterTable')),
    params = list(c('out/fastq/AMA/all_samples'), "all_samples" = c('out/fastq/AMA/all_samples')),
    wildcards = list(),
    threads = 1,
    log = list(),
    resources = list(),
    config = list("out" = 'out', "forward" = 'adapters/forwardPrimers', "pair1" = 'raw_reads/1', "refs" = 'refs', "reverse" = 'adapters/reversePrimers', "ncores" = 4, "pair2" = 'raw_reads/2', "target" = 'AMA')
)
######## Original script #########
# ----------------------------------------- #
